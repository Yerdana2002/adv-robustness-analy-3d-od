# =============================================================================
# focal_decoder.py — refactored for mmdet3d >= 1.1 / v1.4.x
# =============================================================================
# Copyright (c) 2022-2023, NVIDIA Corporation & Affiliates. All rights reserved.
#
# Import changes:
#   HEADS → MODELS, build_loss → MODELS.build, build_bbox_coder → TASK_UTILS.build
#   build_assigner/sampler → TASK_UTILS.build, multi_apply → mmdet.models.utils
#   AssignResult → mmdet.models.task_modules.assigners
#   force_fp32 → REMOVED, circle_nms → mmdet3d.models.layers
#   draw_heatmap_gaussian/gaussian_radius → mmdet3d.models.utils.gaussian
#   xywhr2xyxyr/LiDARInstance3DBoxes → mmdet3d.structures
#   rotation_3d_in_axis → mmdet3d.structures.bbox_3d.utils
#   clip_sigmoid → mmdet3d.models.utils, nms_gpu → nms_bev
#   builder.build_head → MODELS.build
# =============================================================================
import copy
import logging
import numpy as np
...
logger.info('[focal_decoder] Loading module...')

from mmcv.cnn import ConvModule, build_conv_layer
from mmcv.cnn.bricks.transformer import build_transformer_layer_sequence

# --- Manual registration of mmdet transformer layers ---
# In mmcv 2.x, build_transformer_layer_sequence delegates to MODELS.build().
# However, mmdet's DeformableDetrTransformerDecoder and DetrTransformerDecoderLayer
# are NOT decorated with @MODELS.register_module() — they exist only as plain
# classes in mmdet.models.layers. We must register them manually so the config
# type strings resolve correctly.
try:
    ...
    for _cls in [
        ...
        if _MODELS.get(_cls.__name__) is None:
            _MODELS.register_module(module=_cls)
            logger.info(f'[focal_decoder] ✓ Manually registered {_cls.__name__} '
                        f'in MODELS')
except ImportError as e:
    logger.warning(f'[focal_decoder] Could not register mmdet transformer '
                   f'layers: {e}')

# --- Ensure mmdet losses are importable via 'mmdet.' scope prefix ---
# Importing mmdet.models triggers registration of FocalLoss, L1Loss, etc.
try:
    import mmdet.models  # noqa: F401
    logger.info('[focal_decoder] ✓ mmdet.models imported (losses registered)')
except ImportError as e:
    logger.warning(f'[focal_decoder] Could not import mmdet.models: {e}')

logger.info('[focal_decoder] ✓ mmcv.cnn imports')

from mmdet3d.registry import MODELS, TASK_UTILS
logger.info('[focal_decoder] ✓ MODELS, TASK_UTILS from mmdet3d.registry')

from mmdet3d.structures import xywhr2xyxyr, LiDARInstance3DBoxes
logger.info('[focal_decoder] ✓ xywhr2xyxyr, LiDARInstance3DBoxes from mmdet3d.structures')

try:
    from mmdet3d.structures.bbox_3d.utils import rotation_3d_in_axis
    logger.info('[focal_decoder] ✓ rotation_3d_in_axis from mmdet3d.structures.bbox_3d.utils')
except ImportError:
    from mmdet3d.core.bbox.structures.utils import rotation_3d_in_axis
    logger.info('[focal_decoder] ✓ rotation_3d_in_axis (fallback mmdet3d.core)')

from mmdet3d.models.layers import nms_bev, circle_nms
logger.info('[focal_decoder] ✓ nms_bev, circle_nms from mmdet3d.models.layers')

from mmdet3d.models.utils.gaussian import draw_heatmap_gaussian, gaussian_radius
logger.info('[focal_decoder] ✓ draw_heatmap_gaussian, gaussian_radius')

try:
    from mmdet3d.models.utils.clip_sigmoid import clip_sigmoid
except ImportError:
    from mmdet3d.models.utils import clip_sigmoid
logger.info('[focal_decoder] ✓ clip_sigmoid')

from mmdet.models.utils import multi_apply
from mmdet.models.task_modules.assigners import AssignResult
from mmdet.models.task_modules.samplers import PseudoSampler
logger.info('[focal_decoder] ✓ multi_apply, AssignResult, PseudoSampler')
...
logger.info('[focal_decoder] ✓ Project-local imports')

@MODELS.register_module()
class FocalDecoder(nn.Module):

    def __init__(self,
                 num_proposals=128, hidden_channel=128, hidden_channel_roi=512,
                 num_classes=4, num_decoder_layers=1, num_heads=8,
                 initialize_by_heatmap=False, nms_kernel_size=1, bn_momentum=0.1,
                 activation='relu', classaware_reg=False, common_heads=dict(),
                 num_heatmap_convs=2, conv_cfg=dict(type='Conv1d'),
                 norm_cfg=dict(type='BN1d'), bias='auto',
                 loss_cls=dict(type='GaussianFocalLoss', reduction='mean'),
                 loss_bbox=dict(type='L1Loss', reduction='mean'),
                 loss_heatmap=dict(type='GaussianFocalLoss', reduction='mean'),
                 loss_weight_heatmap=1., train_cfg=None, test_cfg=None,
                 bbox_coder=None, num_stage_proposals=None, multiscale=False,
                 multistage_heatmap=False, reuse_first_heatmap=False,
                 extra_feat=False, heatmap_box=False, thin_heatmap_box=False,
                 loss_weight_separate_heatmap=0.2, loss_weight_separate_bbox=0.5,
                 boxpos=None, add_gt_groups=0, add_gt_groups_noise='rect,1',
                 add_gt_groups_noise_box='gt', gt_center_limit=None,
                 add_gt_pos_thresh=100., add_gt_pos_boxnoise_thresh=2.,
                 gt_query_loss_weight=1., bevpos=False, input_img=True,
                 iterbev_wo_img=False, mask_heatmap_mode='poscls',
                 roi_feats=0, roi_dropout_rate=0., roi_expand_ratio=1.,
                 roi_based_reg=False,
                 decoder_cfg=dict(
                    type='DeformableDetrTransformerDecoder', num_layers=6,
                    return_intermediate=False,
                    transformerlayers=dict(
                        type='DetrTransformerDecoderLayer',
                        attn_cfgs=[
                            dict(type='MultiheadAttention', embed_dims=128, num_heads=8, dropout=0.1),
                            dict(type='MultiScaleDeformableAttention', embed_dims=128, num_levels=1, num_points=6, num_heads=8, renorm_z=5.)],
                        feedforward_channels=1024, ffn_dropout=0.1,
                        ffn_cfgs=dict(type='FFN', embed_dims=128, num_fcs=2, act_cfg=dict(type='ReLU', inplace=True)),
                        operation_order=('self_attn', 'norm', 'cross_attn', 'norm', 'ffn', 'norm')))):
        super(FocalDecoder, self).__init__()
        logger.info(f'[FocalDecoder] Building: num_proposals={num_proposals}, num_classes={num_classes}, '
                    f'num_decoder_layers={num_decoder_layers}, multistage_heatmap={multistage_heatmap}')
        ...
        if self.multiscale:
            self.dconv = ConvModule(hidden_channel, hidden_channel, stride=2, kernel_size=3, padding=1, bias=bias, conv_cfg=dict(type='Conv2d'), norm_cfg=dict(type='BN2d'))
            self.dconv2 = ConvModule(hidden_channel, hidden_channel, stride=2, kernel_size=3, padding=1, bias=bias, conv_cfg=dict(type='Conv2d'), norm_cfg=dict(type='BN2d'))
        ...
        # build_bbox_coder → TASK_UTILS.build
        ...
        if self.roi_feats:
            ...
            for i in range(num_roi_layers):
                ...
                fc_list.extend([nn.Linear(pre_channel, chl, bias=False), nn.BatchNorm1d(chl), nn.ReLU(inplace=True)])
        ...
        if self.initialize_by_heatmap:
            layers = []
            layers.append(ConvModule(hidden_channel, hidden_channel, kernel_size=3, padding=1, bias=bias, conv_cfg=dict(type='Conv2d'), norm_cfg=dict(type='BN2d')))
                ...
                if self.multistage_heatmap:
                    ...
                    for i in range(self.multistage_heatmap):
                        if i == 0 and self.reuse_first_heatmap:
                            ...
                            self.heatmap_head_img.append(copy.deepcopy(self.heatmap_head))
                    ...
                    if self.heatmap_box:
                        ...
                        separate_head = {'type': 'DCNSeparateHead', 'init_bias': -2.19, 'final_kernel': 3,
                            ...
                            'in_channels': 64, 'heads': heatmap_common_heads, 'num_cls': 2}
                        if self.train_cfg is not None:
                            self.train_cfg['max_objs'] = 500
                            self.train_cfg['dense_reg'] = 1
                        self.norm_bbox = True
                        # build_loss → MODELS.build
                        self.heatmap_loss_cls = MODELS.build({'type': 'mmdet.GaussianFocalLoss', 'reduction': 'mean'})
                        self.heatmap_loss_bbox = MODELS.build({'type': 'mmdet.L1Loss', 'reduction': 'mean', 'loss_weight': 0.25})
                        for i in range(self.multistage_heatmap):
                            task_heads = nn.ModuleList()
                            if self.thin_heatmap_box:
                                layers = []
                                layers.append(ConvModule(hidden_channel, hidden_channel, kernel_size=3, padding=1, bias=bias, conv_cfg=dict(type='Conv2d'), norm_cfg=dict(type='BN2d')))
                                layers.append(build_conv_layer(dict(type='Conv2d'), hidden_channel, len(separate_num_classes) * 10, kernel_size=3, padding=1, bias=bias))
                                task_heads = nn.Sequential(*layers)
                            else:
                                for num_cls in separate_num_classes:
                                    heads = copy.deepcopy(heatmap_common_heads)
                                    heads.update(dict(heatmap=(num_cls, num_heatmap_convs)))
                                    separate_head.update(in_channels=hidden_channel, heads=heads, num_cls=num_cls)
                                    # builder.build_head → MODELS.build
                                    task_heads.append(MODELS.build(separate_head))
        ...
        logger.info(f'[FocalDecoder] ✓ Built successfully')
    ...
    # generate_gt_groups is algorithmically unchanged from original
    # Only change: uses self.bbox_coder (already built via TASK_UTILS.build)
    ...
    def generate_gt_groups(self, query_feat, query_pos, query_heatmap_score, lidar_feat, lidar_feat_flatten, bev_pos, heatmap,
        ...
        logger.debug(f'[generate_gt_groups] batch_size={batch_size}, max_gts={self.max_num_gts}, groups={self.add_gt_groups}')
        ...
        for batch_idx in range(batch_size):
            ...
            gs_labels = F.pad(gt_labels_3d[batch_idx], pad=(0, self.max_num_gts - self.num_gts[batch_idx]), value=self.num_classes)
            ...
            gs_box = self.bbox_coder.encode(gs_box)
            ...
            gs = gs.to(torch.int64)
            positive_mask = (box_center_noise.norm(dim=1) < self.add_gt_pos_thresh)
            gs_labels[torch.logical_not(positive_mask)] = self.num_classes
        ...
        noise_split = self.add_gt_groups_noise.split(',')

        if len(noise_split) > 2 and noise_split[2] == 'heatmap':
        ...
        elif len(noise_split) > 2 and noise_split[2] == 'heatmapcls':
        ...
        batch_gt_query_feat = batch_gt_query_feat + batch_gt_query_cat_encoding
        ...
        if dense_heatmap_boxes is not None:
            if self.add_gt_groups_noise_box == 'pred':
                batch_gt_heatmap_box = dense_heatmap_boxes.gather(index=(batch_gt_query_labels.clip(max=self.num_classes-1) * H * W + batch_gt_pos[:, :, 1] * H + batch_gt_pos[:, :, 0])[:, None].expand(-1, dense_heatmap_boxes.shape[1], -1), dim=-1)
                ...
                batch_gt_heatmap_box[:, :2, :] = (batch_gt_bev_pos * batch_valid_gt_mask[...,None].float()).transpose(1, 2)
                batch_gt_heatmap_box[:, [2], :] = batch_gt_heatmap_box[:, [2], :] + meta_noise[:, [0]] * batch_gt_heatmap_box[:, [5]].exp()
                batch_gt_heatmap_box[:, [3, 4, 5], :] = torch.log(batch_gt_heatmap_box[:, [3, 4, 5], :].exp() * (1+meta_noise[:, [1, 2, 3]]).clip(min=0.1, max=3.) + 1e-6)
                batch_gt_heatmap_box[:, 6:7, :] = torch.sin(meta_noise[:, 4:5] * np.pi)
                batch_gt_heatmap_box[:, 7:8, :] = torch.cos(meta_noise[:, 4:5] * np.pi)
    ...
    # forward() is algorithmically identical to original
    # Only structural change: uses already-refactored builders
    ...
    def forward(self, pts_inputs, img_inputs, img_metas, gt_bboxes_3d=None, gt_labels_3d=None, **input_kwargs):
        ...
        logger.debug(f'[FocalDecoder.forward] batch_size={batch_size}, lidar_feat={lidar_feat.shape}')
        ...
        if not self.multistage_heatmap:
            ...
            if self.input_img or self.iterbev_wo_img:
                ...
                dense_heatmap_img = self.heatmap_head_img(new_lidar_feat.view(lidar_feat.shape))
            ...
            # Multistage heatmap path - identical logic to original, just uses refactored builders
            dense_heatmap = self.heatmap_head(lidar_feat)
            ...
            for i in range(self.multistage_heatmap):
                    ...
                    if not self.heatmap_box:
                        ...
                        if not self.thin_heatmap_box:
                            for task_id, task in enumerate(self.multi_stage_task_heads[i]):
                                dense_preds.append(task(shared_feat))
                                dense_pred = dense_preds[-1]
                                dense_pred = torch.cat((dense_pred['reg'], dense_pred['height'], dense_pred['dim'], dense_pred['rot'], dense_pred['vel']), dim=1)[:, :, None].expand(-1, -1, self.heatmap_tasks[task_id]['num_class'], -1, -1)
                                dense_heatmap_boxes.append(dense_pred)
                ...
                if self.heatmap_box:
                    ...
                    dense_heatmap_boxes[:, 2:3, :, :] = dense_heatmap_boxes[:, 2:3, :, :].clip(min=-5., max=3.)
                    dense_heatmap_boxes[:, 3:6, :, :] = dense_heatmap_boxes[:, 3:6, :, :].clip(min=np.log(0.5), max=np.log(15))
                    dense_heatmap_boxes[:, 6:8, :, :] = dense_heatmap_boxes[:, 6:8, :, :].clip(min=-1., max=1.)
                ...
                # mask heatmap mode selection - identical to original
                if self.mask_heatmap_mode == 'pos':
                    selected_mask = acc_masks.new_zeros(batch_size, self.num_classes, heatmap.shape[-1])
                    selected_mask.scatter_(index=top_proposals_index[:, None, :].expand(-1, self.num_classes, -1), dim=2, src=acc_masks.new_ones((batch_size, self.num_classes, heatmap.shape[-1])))
                    ...
                    def pos_inside_boxes(query_box, bev_pos, margin, min_bev_dim, margin_ratio=None):
                        ...
                        query_boxes_std[..., 5] = 1000
                        query_boxes_std[..., 2] = -100.
                        temp_bev_pos = self.bbox_coder.decode_center(bev_pos.transpose(1,2))
                        temp_bev_pos = torch.cat([temp_bev_pos, temp_bev_pos.new_zeros(batch_size, 1, bev_pos.shape[1])], dim=1).transpose(1,2)
                        inside_boxes = points_in_boxes_gpu(temp_bev_pos, query_boxes_std[:, :, :7])
                        return inside_boxes
                    ...
                    bev_pos_class[inside_boxes == -1] = self.num_classes
        ...
        if self.training and self.add_gt_groups > 0:
            updated_gt_queries = self.generate_gt_groups(query_feat, query_pos, query_heatmap_score, lidar_feat, lidar_feat_flatten, bev_pos, heatmap, gt_bboxes_3d, gt_labels_3d, dense_heatmap_boxes=dense_heatmap_boxes, query_box=query_box)
        ...
        for i in range(self.num_decoder_layers):
            ...
            if not self.multiscale:
                W, H = lidar_feat.shape[-2:]
                spatial_shapes = torch.as_tensor([[H, W]], dtype=torch.long, device='cuda')  # NOTE: mmdet 3.x expects (h, w)
            ...
            if self.training and self.add_gt_groups > 0:
                ...
                attn_masks = None
            kwargs = dict(spatial_shapes=spatial_shapes, level_start_index=level_start_index, valid_ratios=torch.ones((batch_size, 1, 2), device='cuda'), key_padding_mask=None, self_attn_mask=attn_masks)
            ...
            query_pos_embed = self.pos_embed_learned[i](query_sine_pos)
            ...
            if self.bevpos:
                ...
                bev_pos_embed = self.pos_embed_learned[i](bev_sine_pos)
                pos_lidar_feat_flatten = lidar_feat_flatten + bev_pos_embed.transpose(1,2)
            ...
            if self.roi_feats and query_box is not None:
                ...
                lidar_std_boxes = LiDARInstance3DBoxes(std_boxes, box_dim=std_boxes.shape[-1])
                ...
                if not self.multiscale:
                    roi_feat = F.grid_sample(lidar_feat_flatten.view(batch_size, -1, *lidar_feat.shape[-2:]), grid_points)
                else:
                    ms_roi_feat = []
            ...
            # NOTE: mmdet 3.x decoder expects batch-first: (bs, num_queries, dim)
            # query_feat is (bs, C, num_queries) → need (bs, num_queries, C)
            # pos_lidar_feat_flatten is (bs, C, num_value) → need (bs, num_value, C)
            # query_pos_embed is already (bs, num_queries, C) from MLP output
            query_feat_in = query_feat.permute(0, 2, 1)         # (bs, num_queries, C)
            value_in = pos_lidar_feat_flatten.permute(0, 2, 1)  # (bs, num_value, C)
            query_feat_out, reference_points = self.decoder[i](query=query_feat_in, value=value_in, query_pos=query_pos_embed, reference_points=reference_points, **kwargs)
            # query_feat_out is (bs, num_queries, C) → convert back to (bs, C, num_queries)
            query_feat = query_feat_out.permute(0, 2, 1)
            ...
            if self.roi_based_reg and query_box is not None:
            ...
            logger.debug(f'[FocalDecoder.forward] layer {i}: query_feat={query_feat.shape}')
        ...
        if self.initialize_by_heatmap:
            ...
            ret_dicts[0]['dense_heatmap'] = heatmap_train
        ...
        for key in ret_dicts[0].keys():
            if key in ['dense_heatmap', 'query_heatmap_score', 'multistage_masks', 'gt_query_bbox_targets', 'batch_valid_gt_mask', 'batch_gt_query_labels']:
                new_res[key] = ret_dicts[0][key]

            else:
                if self.training and self.add_gt_groups > 0:
                    gt_offset = self.max_num_gts * self.add_gt_groups
                    new_res[key] = torch.cat([ret_dict[key][:, :, :-gt_offset] for ret_dict in ret_dicts], dim=-1)
                    new_res[key + '_gtgroups'] = torch.cat([ret_dict[key][:, :, -gt_offset:] for ret_dict in ret_dicts], dim=-1)
                else:
    ...
    def get_targets(self, gt_bboxes_3d, gt_labels_3d, preds_dict):
        ...
        logger.debug(f'[FocalDecoder.get_targets] num_pos={num_pos}, matched_ious={matched_ious:.4f}')
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d, preds_dict, batch_idx):
        ...
        for idx_layer in range(self.num_decoder_layers):
            s = self.num_proposals * idx_layer
            e = self.num_proposals * (idx_layer + 1)
            bboxes_tensor_layer = bboxes_tensor[s:e, :]
            score_layer = score[..., s:e]
        ...
        # mmdet 3.x PseudoSampler expects InstanceData, not raw tensors
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        ...
        if self.initialize_by_heatmap:
            if isinstance(preds_dict['dense_heatmap'], (tuple, list)):
                if 'multistage_masks' in preds_dict:
                    ...
                    heatmap = heatmap.repeat(len(preds_dict['dense_heatmap']), 1, 1, 1) * multistage_masks
                else:
        ...
        for idx_layer in range(self.num_decoder_layers):
            ...
            logger.debug(f'[loss] {prefix}: cls={layer_loss_cls.item():.4f}, bbox={layer_loss_bbox.item():.4f}')
        ...
        if self.add_gt_groups > 0:
            ...
            preds_gt_query = torch.cat([preds_dict['center_gtgroups'], preds_dict['height_gtgroups'], preds_dict['rot_gtgroups'], preds_dict['dim_gtgroups']], dim=1)

            if 'vel' in preds_dict.keys():
                preds_gt_query = torch.cat([preds_gt_query, preds_dict['vel_gtgroups']], dim=1)
            preds_gt_query = preds_gt_query.permute(0, 2, 1)
            score_gt_query = preds_dict['heatmap_gtgroups'].permute(0, 2, 1).reshape(-1, self.num_classes)
            ...
            loss_dict['gt_query_loss_box'] = self.loss_bbox(preds_gt_query, gt_query_bbox_targets, gt_query_reg_weights, avg_factor=max(sum(self.num_gts) * self.add_gt_groups * num_gt_groups_layers, 1)) * self.gt_query_loss_weight
            gt_query_label_weights = batch_valid_gt_mask.repeat(1, num_gt_groups_layers).reshape(-1)
            loss_dict['gt_query_loss_cls'] = self.loss_cls(score_gt_query, batch_gt_query_labels.reshape(-1), gt_query_label_weights, avg_factor=max(sum(self.num_gts) * self.add_gt_groups * num_gt_groups_layers, 1)) * self.gt_query_loss_weight
            ...
            for stage_id, dense_preds in enumerate(multistage_bev_preds):
                ...
                for task_id, dense_pred in enumerate(dense_preds):
                    ...
                    if not self.thin_heatmap_box:
                        ...
                        num_pos_h = heatmaps[task_id].eq(1).float().sum().item()
                        loss_heatmap = self.heatmap_loss_cls(dense_pred['heatmap'], heatmaps[task_id], weight=classwise_ignore_mask, avg_factor=max(num_pos_h, 1))
                    ...
                    mask *= isnotnan
                    code_weights = self.train_cfg.get('code_weights', None)
                    bbox_weights_h = mask * mask.new_tensor(code_weights)
                    ...
                    bbox_weights_h = bbox_weights_h * box_ignore_mask
                    loss_bbox = self.heatmap_loss_bbox(pred, target_box, bbox_weights_h, avg_factor=(num + 1e-4))
        ...
        for layer_id, preds_dict in enumerate(preds_dicts):
            ...
            for i in range(batch_size):
                ...
                if self.test_cfg['nms_type'] is not None:
                    ...
                    for task in self.tasks:
                        ...
                        if task['radius'] > 0:
                            if self.test_cfg['nms_type'] == 'circle':
                                ...
                                # nms_gpu → nms_bev
                                task_keep_indices = nms_bev(boxes_for_nms, top_scores, thresh=task['radius'], pre_maxsize=self.test_cfg['pre_maxsize'], post_max_size=self.test_cfg['post_maxsize'])
                        else:
                    ...
                    if len(boxes3d) > 200:
                        logger.warning('get more than 200 boxes, save top 200 only!')
    ...
    def get_heatmap_targets(self, gt_bboxes_3d, gt_labels_3d):
        heatmaps, anno_boxes, inds, masks = multi_apply(self.get_heatmap_targets_single, gt_bboxes_3d, gt_labels_3d)
    ...
    def get_heatmap_bboxes(self, preds_dicts, img_metas, img=None, rescale=False):
        ...
        for layer_id, preds_dict in enumerate(preds_dicts):
            ...
            #temp = self.bbox_coder.decode(batch_score, rot.clone(), dim.clone(), center.clone(), height.clone(), vel.clone(), filter=True) orig
...
logger.info('[focal_decoder] ✓ Registered FocalDecoder to MODELS')
logger.info('[focal_decoder] ✓ Module fully loaded')

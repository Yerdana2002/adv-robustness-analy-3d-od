...
#SBATCH --job-name=attack_focalformer
#SBATCH --tmp=800G
#SBATCH --signal=B:USR1@1800
# =============================================================================
# FocalFormer3D-L adversarial attack on nuScenes val. nuScenes ONLY.
#
...
# attack_focalformer_l_resume.sh can stage back in. rescue_running_attack.sh
...
WORK_DIR=$PROJECT_ROOT/work_dirs/attack_focalformer_l_${SLURM_JOB_ID}
mkdir -p "$WORK_DIR"
...
echo "============================================================"
echo "FocalFormer3D-L adversarial attack (nuScenes val)"
...
# exists so attack_focalformer_l_resume.sh can pick it up.
persist () {
    local dest=$1 n
...
on_wall () {
    ...
    persist "$PROJECT_ROOT/data/adv_points_focalformer_l_channel_partial${SLURM_JOB_ID}.tar"
    echo "!! resume: sbatch --export=ALL,RESUME_JOB=${SLURM_JOB_ID}$([ "$LOSS_SIGN" != "1.0" ] && echo ",LOSS_SIGN=$LOSS_SIGN") attack_focalformer_l_resume.sh"
    exit 1
}
trap on_wall USR1

...
#SBATCH --job-name=ff_lc_resume
#SBATCH --tmp=800G
#SBATCH --signal=B:USR1@1800
# =============================================================================
# Resume an interrupted FocalFormer3D-L attack from rescued partial tars.
#
#   sbatch attack_focalformer_lc_resume.sh
#   sbatch --export=ALL,LOSS_SIGN=0.0 attack_focalformer_lc_resume.sh
#
...
# No default on purpose. This inherited 18882811 -- the LiDAR-only attack's
# job id -- which would stage L partial tars into an LC run and silently
# produce a mixed set that nothing downstream could detect.
RESUME_JOB=${RESUME_JOB:?set RESUME_JOB=<jobid of the LC attack being resumed>}
...
PARTIAL_GLOB="$PROJECT_ROOT/data/adv_points_focalformer_lc_channel_partial${RESUME_JOB}"

WORK_DIR=$PROJECT_ROOT/work_dirs/attack_ff_lc_resume_${SLURM_JOB_ID}
mkdir -p "$WORK_DIR"
...
echo "============================================================"
echo "FocalFormer3D-LC attack RESUME (nuScenes val)"

...
#SBATCH --job-name=ff_lc_adv_tables
#SBATCH --tmp=800G
# =============================================================================
...
# -------------------------
# The attack perturbs LiDAR points only; the six camera images stay pristine.
# So focalformer-lc vs focalformer-l measures the camera branch rescuing a
# corrupted LiDAR input -- NOT fusion robustness in general. Unlike the
# BEVFusion pair, though, these two share a checkpoint lineage, the same 5980
# frames and identical filters, so the comparison is at least within one
# architecture rather than across two.
#
...
SETS=(
  "lidar-cam|$PROJECT_ROOT/data/adv_points_focalformer_lc_channel.tar|focalformer-lc|advlidar"
)

OUT_DIR=$PROJECT_ROOT/work_dirs/ff_lc_adv_tables_${SLURM_JOB_ID}
...
echo "============================================================"
echo "FocalFormer3D-LC adversarial metrics + AI@x tables (nuScenes)"
...
run_eval () {   # tag
    ...
    echo "Eval [$tag]"
    echo "------------------------------------------------------------"
    # ann_file must be overridden: FocalFormer3D_LC_test.py ships pointing at
...
BD=(
  "$TABLE_DIR/tables_focalformer-lc_lidar-cam.pkl"
...
echo "============================================================"
printf '%-24s %s\n' "clean focalformer-lc" "$(metric_of clean)"
for entry in "${SETS[@]}"; do
    NAME=$(echo "$entry" | cut -d'|' -f1)
    printf '%-24s %s\n' "adv $NAME" "$(metric_of "adv_$NAME")"
done
echo ""
echo "Reference for the clean row: mAP 0.705 / NDS 0.731 (measured 0.7025/0.7282 in 19039335)."

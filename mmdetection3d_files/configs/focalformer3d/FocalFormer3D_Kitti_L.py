...
sparse_shape = [grid_size[2], grid_size[1], grid_size[0]]   # Z, Y, X
...
# points loader (single-sweep, KITTI)
train_pipeline = [
    dict(
...
data = dict(
    ...
    workers_per_gpu=0,
    train=dict(
        type='RepeatDataset', # CBGS Dataset does not work for Kitti, so we use this instead
        times=2,
        dataset=dict(
            type=dataset_type,
            data_root=data_root,
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        decoder_cfg=dict(
            ...
            transformerlayers=dict(
                ...
                attn_cfgs=[
                    dict(type='MultiheadAttention', embed_dims=inter_channel, num_heads=8, dropout=0.1),
                    dict(type='MultiScaleDeformableAttention', embed_dims=inter_channel, num_levels=3, num_points=4, num_heads=8,)
                ],
    ...
    # train & test cfg adjusted for KITTI computed grid sizes
    train_cfg=dict(
        pts=dict(
            dataset='KITTI',
            assigner=dict(
            ...
            min_radius=2,
            # computed grid size: [x_count, y_count, z_count]
            grid_size=grid_size,
            voxel_size=voxel_size,
            out_size_factor=out_size_factor,

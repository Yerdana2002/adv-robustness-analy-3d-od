...
class MVXTwoStageDetector(Base3DDetector):
    ...
    def voxelize(self, points):
        ...
        #     # unwrap list elements that are DataContainers
        #     if len(points) > 0 and isinstance(points[0], DC):
        #         points = [p.data for p in points]

        #     # after unwrapping, each element should be a Tensor
        #     # handle accidental nested list/tuple: [[Tensor]] -> [Tensor]
        #     if len(points) == 1 and isinstance(points[0], (list, tuple)) and len(points[0]) > 0 and torch.is_tensor(points[0][0]):
        #         points = list(points[0])

        #     assert all(torch.is_tensor(p) for p in points), \
        #         f"Expected List[Tensor] for points, got {[type(p) for p in points]}"
